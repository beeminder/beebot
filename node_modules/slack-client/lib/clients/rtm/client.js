...
RTMClient.prototype._onStart = function _onStart(err, data) {
  ...
  if (err || !data.url) {
    ...
    if (data && contains(UNRECOVERABLE_RTM_START_ERRS, data.error)) {
      errMsg = 'unrecoverable failure connecting to the RTM API: ' + data.error;
 ...
 * @param optReason
 */
RTMClient.prototype.disconnect = function disconnect(optReason) {
  this.emit(CLIENT_EVENTS.DISCONNECT, optReason);
