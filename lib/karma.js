...
var handleMessage = function(rtm, msg) {
  redis.get(rkey(msg.team, "on"), function(err, obj) {
    if (obj && !err) {
      var regex = /(\S+)(\+\+|--)\b/g
      var match = null
      while ((match = regex.exec(msg.text)) !== null) {
        ...
        if (matchname.match(/^\<\@(.*)\>/)) {
          ...
          webClient.users.info(userId, function(error, response) {
            changeKarmaAndPost(rtm, msg.team, msg.channel,
                               response.user.name, delta)
          })
        } else {
          if (match[2] === "++") {
            changeKarmaAndPost(rtm, msg.team, msg.channel, matchname, 1)
          } else if (match[2] === "--") {
            changeKarmaAndPost(rtm, msg.team, msg.channel, matchname, -1)
          }
        }
...
var handleSlash = function(req, resp) {
  ...
  if (text === "on") {
    redis.set(rkey(team_id, "on"), 1, function(err, obj) {
      ...
      console.log(
        `DEBUG err/obj = ${JSON.stringify(err)}/${JSON.stringify(obj)}`)
      beebot.startBot(team_id, rurl)
