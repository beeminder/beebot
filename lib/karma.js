...
var handleMessage = function(rtm, message) {
  redis.get("beebot.karmabot." + message.team + ".on", function(err, obj) {
    if (obj && !err) {
      ...
      while ((match = reg.exec( message.text )) != null)
      {
        if (match[2] === "++") {
          redis.zincrby("beebot.karmabot." + message.team, 1, match[1],
          function(err, obj) {
            rtm.send({
              id: 1,
              type: "message",
              channel: message.channel,
              text: match[1] + " has " + obj + " karma"
            });
          })
        } else if (match[2] === "--") {
          redis.zdecrby("beebot.karmabot." + message.team, 1, match[1],
          function(err, obj) {
            rtm.send({
              id: 1,
              type: "message",
  ...
  if (text === "on") {
    ...
    function(obj, err) {
      if (!err && obj.length > 0) {
        ...
        botils.whisp(res, "It looks like you don't have karma turned on for "
        + "this team. You can turn it on with `/karma on`");
      }
    });
  } else {
    help(res);
