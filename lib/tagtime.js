...
var handleSlash = function(req, res) {
  ...
  if (text === "on") {
    redis.sadd("beebot.tagtime", team_id + "-" + user_id, function(err, obj) {
      botils.whisp(res, "TagTime is now watching you!");
    });
  } else if (text === "off") {
    redis.srem("beebot.tagtime", team_id + "-" + user_id, function(err, obj) {
...
var handlePing = function(req, res) {
  ...
  redis.smembers("beebot.tagtime", function(err, obj) {
    obj.forEach(function(key) {
      ...
      var dm = rtm.dataStore.getDMById(key.split("-")[1]);
