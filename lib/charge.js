...
var handleChargeCheck = function() {
  redis.zrangebyscore("beebot.charges",
    ...
    function(err, obj) {
      obj.forEach(function(e) {
        ...
        charge(null, charge.user_id, charge.team_id, charge.amount, charge.reason);
        ...
        redis.zremrangebyscore("beebot.charges", charge.charge_at,
          charge.charge_at,
          function(err, obj) {
            redis.zadd("beebot.charges", charge_at, JSON.stringify(charge),
...
var scheduleCharge = function(res, charge) {
  ...
  redis.zadd("beebot.charges", charge.charge_at, JSON.stringify(charge),
    function(err, obj) {
      var message = "Will charge " + user +" $"+ amount +" in "+ delay +
        " minutes. ";
...
var runCharge = function(res, charge) {
  // send charge to Beeminder now. use **charge as the command to avoid
  // collision with a "charge" goalname.
  https.get("https://www.beeminder.com/slackbot?text="
    + encodeURIComponent("**charge") + "&team_id=" + charge.team_id +
    "&user_id=" + charge.user_id + "&note=" + charge.reason + "&amount=" + charge.amount,
    function(beemRes) {
      var resText = '';
...
var handleSlash = function(req, res) {
  ...
  if (text === "help") {
    ...
    // TODO: list all pending charges
  } else if (text === "cancel") {
    // TODO: cancel all charges for the user.
  } else if (text.match(/^cancel/)) {
    var reason = text.split(" ").slice(1).join(" ");
    // TODO: find any charge for this user with this reason and cancel it
    ...
    if (text.match(/^[\d\.]+\sin\s([\d\.])+\s+(.*)/)) {
      var delay = parseInt(text.match(/^[\d\.]+\sin\s([\d\.])+\s+(.*)/)[1]);
...
module.exports = {
  ...
  handleChargeCheck: handleChargeCheck
}
