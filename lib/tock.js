...
var updateBeeminder = function(teamId, userId, slug, tocktext) {
  ...
  https.get("https://www.beeminder.com/slackbot?command="
    + encodeURIComponent(slug + " ^ " + " 1 \"" + tocktext + "\"") 
    + "&team_id=" + teamId 
    + "&user_id=" + userId
...
var help = function(res) {
  ...
  + "`/tock beemind` list connected Beeminder goal (if any).\n"
...
var beemind = function(res, user, chan, team_id, goalname=null) {
  if (goalname) {
    redis.set("beebot.tockbot.links."+ team_id + "." + user, goalname,
      function(err, obj) {
        botils.shout(res, 
                     "Linked tocks for "+ user +" to Beeminder goal "+ goalname)
    });
  } else {
    redis.get("beebot.tockbot.links."+ team_id + "." + user,
      function(err, goalname) {
        if (goalname) {
          botils.shout(res, 
            "Tocks for " + user + " are linked to Beeminder goal " 
                       + user + "/" + goalname);
        } else {
          botils.shout(res, "No Beeminder goal linked");
        }
    });
  }
}
...
var setChannelLength = function(res, chan, length) {
  redis.set("beebot.tockbot.channels." + chan + ".length", length,
    function(err, obj) {
      botils.shout(res, 
                  "New tocks are now "+ length +" minutes long. Active tocks "+
                  "are unaffected.")
  });
}
    ...
    function(err, obj) {
      ...
      if (foundTock) {
        ...
        redis.get("beebot.tockbot.channels."+ chan + ".length", (err, obj) => {
          ...
          // that the fact that it's in milliseconds will make it unique to 
          // retrieve.
          redis.zadd("beebot.tockbot.tocks." + chan, dueby, 
            JSON.stringify(tock),
            function(err, obj) {
