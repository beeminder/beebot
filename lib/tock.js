              ...
              redis.get(rkey("links", team_id, user),
                (err, goalname) => {
                  if (goalname) {
                    ...
                    botils.shout(res, `${user} SUCCESS :tada:: ${tock.text}`)
                  }
                }
              )
            }
...
var unlink = (res, user, team_id) => {
  redis.del(rkey("links", team_id, user),
    (err, obj) => {
      botils.whisp(res, "Unlinked tocks from Beeminder for " + user)
  })
}
...
var beemind = (res, user, chan, team_id, goalname=null) => {
  if (goalname) {
    redis.set(rkey("links", team_id, user), goalname,
      (err, obj) => {
        botils.whisp(res, "Linked tocks for "+user+" to Beeminder goal "+goalname)
    })
  } else {
    redis.get(rkey("links", team_id, user),
      (err, goalname) => {
        if (goalname) {
          ...
          botils.whisp(res, "No Beeminder goal linked");
        }
    });
  }
}
...
var setChannelLength = (res, chan, length) => {
  redis.set(rkey("channels", chan, "length"), length,
    (err, obj) => {
      botils.shout(res, "New tocks are now "+ length +
...
var echoChannelLength = (res, chan, chanName) => {
  redis.get(rkey("channels", chan, "length"), (err, length) => {
    botils.whisp(res, "Tocks in " + chanName + " are " + length + " minutes long.")
  })
}
