...
var handleMessage = function(rtm, msg) {
  console.log(`DEBUG4: handleMessage ${msg.text}`)
  var text = msg.text
  var regexpString = "<@" + rtm.activeUserId + ">"
  if (msg.text.match(new RegExp(regexpString))) {
    // remove the @-mention of the bot from the message
    var tokenized = msg.text.split(/\s/)
  ...
  https.get("https://www.beeminder.com/slackbot?command="
    + encodeURIComponent(text) + "&team_id=" + msg.team 
                               + "&user_id=" + msg.user,
    function(res) {
      var resText = ''
...
var startBot = function(teamId, rurl=null) {
  redis.hgetall("beebot.teamid." + teamId, function(err, obj) {
    ...
    rtm.on('message', function(msg) {
      ...
      if (msg.channel.match(/^D/) && msg.team && !msg.command) {
        handleMessage(rtm, msg)
      }
      if (msg.text.match(new RegExp(/\+\+|--/))) {
        karmabot.handleMessage(rtm, msg)
      }
    })
